...
{block name='frontend_listing_box_article_actions'}
    <div class="actions">
        ...
        {* Buy now button *}
        {if !$sArticle.priceStartingFrom &&!$sArticle.sConfigurator && !$sArticle.variants && !$sArticle.sVariantArticle && !($sArticle.laststock == 1 && $sArticle.instock <= 0) && !($sArticle.notification == 1 && {config name="deactivatebasketonnotification"} == 1)}
